...
No imported libraries are needed for this notebook.

This notebook demonstrates some rudimentary data exploration techniques using the base R language:
...
Missing data is encoded with *NA*. We can count them with the sum() and is.na() functions, nested. The is.na() function returns a TRUE FALSE vector. Then the sum functions adds the TRUE values because TRUE=1 and FALSE=0.
...
These plots are very simple but convey important information. If you want to make more visually appealing graphs there are many options to modify the point color, symbol, size, change the labels, and much more.
...
We can check for correlations between columns by creating a table with the cor() function or visually look for correlations by plotting pairs(). The **use="complete"** option tells it to ignore NAs.
